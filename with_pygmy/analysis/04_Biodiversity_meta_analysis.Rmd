...
host_colors <- c(
  ...
  "Centropyge flavissima" = "purple",
  "Pygmy Angelfish Hybrid" = "green",
  "Centropyge eibli" = "darkolivegreen")
...
# Save the file
#save(iNEXT_data, file = "with_pygmy/data/04_Biodiversity/iNEXT_data.RData")
```

# Evaluate the Diversity! 
```{r load-div}
load("with_pygmy/data/04_Biodiversity/iNEXT_data.RData")
str(iNEXT_data)
typeof(iNEXT_data)
```
...
iNEXT_color_df <- 
  ...
  mutate(Run = gsub(Run, pattern = "[.]", replace = "-"),
        Run = gsub(Run, pattern = "X",  replace = ""))
  # Merge with metadata
  left_join(metadata_df, by = "Run") %>%
   #Merge with colors for plotting with ggiNEXT
  left_join(data.frame(host_colors = host_colors,
            host = names(host_colors)),
            by = "Host")
```
