...
load("combined_data_with_pygmy/data/03_Phylogenetic_Tree/phytree_preprocessed_physeq.RData")
unrooted_physeq
midroot_physeq
```
...
raw_rooted_physeq <-
  ...
  subset_samples(Run != "G199_IV") %>%
  subset_samples(Run != "SRR17060827") %>%
  subset_samples(Run != "SRR17060829") %>%
  subset_samples(Run != "SRR17060839") %>%
  subset_samples(Run != "SRR17060845") %>%
 subset_samples(Run != "SRR21492551.fastq_NA") %>%
...
```{r combined-wUnifrac, fig.width=10, fig.height=3.5}
(wUNI_host_nmds + theme(legend.position = "right")) + 
  (wUNI_gut_section_nmds + theme(legend.position = "right"))
```
