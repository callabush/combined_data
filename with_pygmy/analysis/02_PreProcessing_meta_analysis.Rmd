...
load("combined_data_with_pygmy/data/01_DADA2/ASV_counts.RData")
...
##  Taxonomy Table
```{r load-tax-table}
tax_df <- read.table("combined_data_with_pygmy/data/01_DADA2/ASV_taxonomy.tsv",
                     sep = "\t", skip = 1)
head(tax_df)
...
```{r load-metadata}
# Load in metadata
metadata_df <- read.csv("with_pygmy/data/combined_metadata.csv")
...
save(raw_physeq, file = "combined_data_with_pygmy/data/02_PreProcessing/raw_physeq.RData")
```
...
write.csv(df, 'combined_data_with_pygmy/data/control_taxa.csv')

# Would only like to keep ASV 7, everything else is trash
controlASVs_toREMOVE <- 
  dplyr::setdiff(control_ASVs$ASV, c("ASV_23"))
# double check 
...
save(raw_preprocessed_physeq,
     file = "combined_data_with_pygmy/data/02_PreProcessing/raw_preprocessed_physeq.RData")
