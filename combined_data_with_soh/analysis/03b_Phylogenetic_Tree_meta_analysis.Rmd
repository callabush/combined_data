...
# Load in the tree!
unrooted_tree <- 
  read.tree("combined_data_with_soh/data/03_Phylogenetic_Tree/ASVs_unrooted.tree")
unrooted_tree
str(unrooted_tree)
```
...
# Recreate a phyloseq object without ASV_37211
unrooted_physeq_rm3711 <-
  unrooted_physeq %>%
  pop_taxa(., "ASV_3711")

# Intuition Check
ntaxa(unrooted_physeq) - ntaxa(unrooted_physeq_rm3711)

# Visually inspect
ggtree(unrooted_physeq_rm3711) +
  geom_tippoint(mapping = aes(color = Kingdom))+
  scale_color_manual(values = c("goldenrod1", "cornflowerblue", "gray"))
```
...
```{r midroot-tree}
# Is the tree rooted? 
new_unrooted_tree <- phy_tree(unrooted_physeq_rm3711)
...
physeq_rm3711 <-
  raw_preprocessed_physeq %>%
  subset_taxa(ASV != "ASV_3711")

# Merge tree with the new physeq_rm456
midroot_physeq <-
  merge_phyloseq(physeq_rm3711, midpoint_rooted_tree)
midroot_physeq
...
save(list = c("unrooted_physeq", "midroot_physeq"),
     file = "combined_data_with_soh/data/03_Phylogenetic_Tree/phytree_preprocessed_physeq.RData")
```
