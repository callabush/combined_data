...
```{r load-host-colors}
host_colors <- c(
  "Pomacanthus sexstriatus" = "magenta",
  "Kyphosus sydneyanus" = "deeppink4",
  "Kyphosus cinerascens" = "blue3",
  "Kyphosus vaigiensis" = "goldenrod",
  "Pomacanthus annularis" = "olivedrab")
```
...
load("combined_data_with_soh/data/03_Phylogenetic_Tree/phytree_preprocessed_physeq.RData")
unrooted_physeq
midroot_physeq
```
...
scaled_total_seqs_df %>%
  ...
  labs(title = "Scaled Sequencing Depth at 526") + 
  theme_bw()
...
- Axis 2 = ~13% of variation 

This means we explain 27% of the variation in the data in these two axes. 
...
This means we explain 45% of the variation in the data in these two axes, which is almost exactly the same as the previous plot with the Sorensen Dissimilarity. 
...
- Axis 1 = ~56% of variation  
- Axis 2 = ~13% of variation 

This means we explain *69*% of the variation in the data in these two axes (!!!), which is significantly more than the previous plots with the taxonomic dissimilarity measures. 
...
wUNI_gut_section_nmds <- 
  ...
  scale_color_manual(values = c("blue3", "deeppink4", "goldenrod","olivedrab",  "magenta")) + 
...
We can see from above the plot that the stress value is ~0.115, which is close to being on the edge of acceptable.
